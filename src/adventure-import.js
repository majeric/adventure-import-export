...
export default class AdventureModuleImport extends FormApplication {
    ...
    async getData() {
        ...
        try {
            if (Helpers.verifyPath("data", importpath)) {
                data = await Helpers.BrowseFiles("data", importpath, {
                    bucket: null,
                    extensions: [".fvttadv", ".FVTTADV"],
                    wildcard: false
                })
    ...
    async _dialogButton(event) {
        ...
        if (action === "import") {
            ...
            try {
                ...
                if (adventure.system !== game.system.id) {
                ...
                if (folders) {
                    ...
                    if (!maintainFolders) {
                        ...
                        await Helpers.asyncForEach(importTypes, async importType => {
                            ...
                            CONFIG.AIE.TEMPORARY.folders[importType] = itemfolder._id;
                        })
                    } else {
                    ...
                    let firstLevelFolders = folders.filter(folder => {
                            ...
                            try {
                                ...
                                switch (obj.documentName) {
                                    case "Scene":
                                        ...
                                        await Helpers.asyncForEach(obj.data.tokens.contents, async token => {
                                            if (token.actorId) {
                                                ...
                                                if (actor) {
                                                    await obj.updateEmbeddedDocuments("Token", [{
                                                        _id: token.id,
                                                        actorId: actor._id
                                                    }]);
                                                }
                                        ...
                                        await Helpers.asyncForEach(obj.data.notes.contents, async note => {
                                            if (note.entryId) {
                                                ...
                                                if (journalentry) {
                                                    await obj.updateEmbeddedDocuments("Note", [{
                                                        _id: note.id,
                                        ...
                                        let sceneJournal = Helpers.findEntityByImportId("journal", obj.journal);
                                        if (sceneJournal) {
                                            updatedData["journal"] = sceneJournal?._id;
                                        }
                                        let scenePlaylist = Helpers.findEntityByImportId("playlists", obj.playlist);
                                        if (scenePlaylist) {
                                    ...
                                    case "RollTable":
                                        ...
                                        await Helpers.asyncForEach(obj.results, async (result, index) => {
                                            switch (result.type) {
                                                ...
                                                case 2:
                                                    ...
                                                    const pack = await game.packs.get(obj.collection);
                                                    if (!pack.locked && !pack.private) {
                                                        let content = await pack.getContent();

                                                        let compendiumItem = content.find(contentItem => {
                                                            return contentItem.flags?.importid === obj.resultId;
                                                        });

                                                        if (compendiumItem) {
                                                            updatedData.results[index].resultId = compendiumItem.id;
                                                        }
                                                    }
                                                    break;
                                                default:
                                        ...
                                        break;
                                    default:
                                        // this is where there is a reference in one of the fields
                                        rawData = JSON.stringify(obj.toObject());
                                                ...
                                                try {
                                                    ...
                                                    if (!pack.locked && !pack.private) {
                                                        ...
                                                        if (!compendiumItem) {
                                                            ...
                                                            newObj["_id"] = `${p}.${name}.${compendiumItem.id}`;
                                                        }
                                                    }
                                                } catch (err) {
                                        ...
                                        const diff = Helpers.diff(obj.toObject(), updatedDataUpdates);
    ...
    async _importFile(type, zip, adventure, folders) {
        ...
        await Helpers.asyncForEach(dataFiles, async (file) => {
            ...
            if (data.img) {
                const importedPath = await Helpers.importImage(data.img, zip, adventure);
                data.background = data.background || {};
                data.background.src = importedPath;
                delete data.img;
            }
            ...
            switch (typeName) {
                case "Scene":
                    if (!Helpers.findEntityByImportId("scenes", data._id)) {
                        ...
                        this._itemsToRevisit.push(scene.uuid);
                    }
                    break;
                case "Actor":
                    if (!Helpers.findEntityByImportId("actors", data._id)) {
                        try {
                            ...
                            // // Patch missing/invalid item img fields
                            // if (Array.isArray(data.items)) {
                            //     for (const item of data.items) {
                            //         if (item.img && !/\.(png|jpg|jpeg|webp|svg)$/i.test(item.img)) {
                            //             Helpers.logger.warn(`Fixing invalid item image: ${item.img}`);
                            //             item.img += ".png"; // or assign a fallback like "icons/svg/item-bag.svg"
                            //         }
                            //     }
                            // }
                            ...
                            if (!actor) {
                                Helpers.logger.warn("Actor creation failed for:", data);
                                return;
                            }

                            // Fix token data reference (FVTT v10+)
                            await actor.update({
                                "prototypeToken.actorId": actor.id
                            });

                            if (needRevisit) {
                                this._itemsToRevisit.push(actor.uuid); // More robust than Actor.${id}
                            }
                        } catch (err) {
                            Helpers.logger.error("Error importing actor:", err, data);
                        }
                    }
                    break;
                ...
                case "Item" :
                    ...
                    break;
                case "Journal" :
                    if (!Helpers.findEntityByImportId("journal", data.id)) {
                    ...
                    break;
                case "Table" :
                    if (!Helpers.findEntityByImportId("tables", data.id)) {
